...
Class AvandApiRequestHandler extends ApiRequestHandler {
    /**
    ...
    public static function Request($baseUri, $method, $relativeUri, $option, $restFull = false, $optionHasArray = false) {
        ...
        if(isset($result['result']['result'])) {
            $avandResult =  json_decode($result['result']['result'], true);
            $result['result']['result'] = $avandResult;
        } else {
            $avandResult = '';
        }
        ...
        // handle error from Notification
        }elseif ((isset($result['result']['statusCode']) && ($result['result']['statusCode'] < 200 || $result['result']['statusCode'] >= 300)) || (isset($avandResult['hasError']) && $avandResult['hasError']) || (isset($avandResult['errorCode']) && ($avandResult['errorCode'] != 0))){
                $message = !empty($avandResult['message']) ? $avandResult['message'] : "Some unhandled error has occurred!";
                $errorCode = isset($avandResult['errorCode']) ? $avandResult['errorCode'] : PodException::SERVER_UNHANDLED_ERROR_CODE;
                throw new PodException($message, $errorCode,null, $result);
        }
        return $result;
    }
